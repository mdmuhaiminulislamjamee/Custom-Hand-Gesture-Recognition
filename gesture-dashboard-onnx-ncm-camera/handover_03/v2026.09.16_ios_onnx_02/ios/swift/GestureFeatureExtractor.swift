...
public struct GestureFeatureExtractor {
    ...
    public static func isFistPose(_ points: [SIMD2<Float>]) throws -> Bool {
        guard points.count == 21 else { throw GestureFeatureError.invalidLandmarkShape }
        ...
        return (nonThumb.max() ?? 1.0) <= 0.55 && mean <= 0.38 && thumb <= 0.85
    }
    ...
    public static func isThumbDownPose(
        ...
        let strict = thumbExtension >= 0.60 && meanNonThumb <= 0.72 && downScore >= 0.55
        let modelAssisted = modelConfidence >= 0.95
            && thumbExtension >= 0.45
            && downScore >= 0.50
            && lead >= 0.20
    ...
    public static func runtimePoseAllowed(
        ...
        case "fist":
            return try isFistPose(landmarksXY)
        case "thumb_down":
