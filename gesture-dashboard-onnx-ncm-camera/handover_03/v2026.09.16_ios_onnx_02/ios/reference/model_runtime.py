...
class OnnxClassifier:
    ...
    def __init__(
        ...
        if list(metadata.get("output_class_order", [])) != config.class_names:
            raise ValueError("ONNX output class order differs from the ten-command contract.")
        ...
        if len(probability_indexes) != 1:
            ...
            raise ValueError(
                "Could not identify the unique ten-class ONNX probability output; "
    ...
    def process_frame(
        ...
        *,
        ...
        with self._lock:
            ...
            world_landmarks = (
                self.detector.world_landmarks()
                if hasattr(self.detector, "world_landmarks")
                else None
            )
        ...
        return {
            ...
            "world_landmarks": (
                np.asarray(world_landmarks, dtype=np.float32).round(7).tolist()
