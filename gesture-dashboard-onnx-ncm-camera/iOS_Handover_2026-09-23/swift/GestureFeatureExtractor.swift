...
public struct GestureFeatureExtractor {
    ...
    public static func runtimePrediction(
        from probs: [Float],
        landmarksXY: [SIMD2<Float>]? = nil
    ) -> String? {
        ...
        let raw = classNames[maxIdx]
        if let landmarksXY,
           raw != "like",
           raw != "thumb_down",
           (try? isForeshortenedFistPose(landmarksXY)) == true {
            return "fist"
        }
        return raw
    }
    ...
    public static func isFistPose(
        ...
        return strict || modelAssisted || (try isForeshortenedFistPose(points))
    }

    /// Detects a raised, camera-facing fist even when perspective hides the
    /// PIP/DIP bend. Mirroring and in-plane rotation do not change the result.
    public static func isForeshortenedFistPose(
        _ points: [SIMD2<Float>]
    ) throws -> Bool {
        guard points.count == 21 else { throw GestureFeatureError.invalidLandmarkShape }
        let mcps = [points[5], points[9], points[13], points[17]]
        let tips = [points[8], points[12], points[16], points[20]]
        let palmCenter = mcps.reduce(SIMD2<Float>(repeating: 0), +) / 4.0
        let palmAxis = palmCenter - points[0]
        let palmScale = norm(palmAxis)
        guard palmScale >= eps else {
            throw GestureFeatureError.degenerateGeometry("Degenerate palm axis for Fist.")
        }
        let palmUnit = palmAxis / palmScale
        let curlbacks = zip(mcps, tips).map { mcp, tip in
            let mcpProjection = simd_dot(mcp - points[0], palmUnit)
            let tipProjection = simd_dot(tip - points[0], palmUnit)
            return (mcpProjection - tipProjection) / palmScale
        }
        let retractedCount = curlbacks.filter { $0 >= 0.05 }.count
        let meanCurlback = curlbacks.reduce(0, +) / Float(curlbacks.count)
        let maximumTipRadius = tips.map { dist($0, palmCenter) / palmScale }.max() ?? .infinity
        let thumbRadius = dist(points[4], palmCenter) / palmScale
        return retractedCount == 4
            && meanCurlback >= 0.16
            && maximumTipRadius <= 0.85
            && thumbRadius <= 0.85
    }
